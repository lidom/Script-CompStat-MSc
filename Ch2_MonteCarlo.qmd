...
(b) What is the classic Monte Carlo integration estimator, $\hat{\theta},$ of $\theta$? Compute one estimate of $\theta$ using a sample size of $n=100.$
...
$$
where $Y=\frac{h(X)f(X)}{g(X)}$ and $X\sim g.$ 

Why is the density function $g\sim\mathcal{N}(4,1)$ potentially good choice? 
...
(f) Compare the variance of the importance sampling estimator with the variance of the classical Monte Carlo integration estimator of the probability $P(X\geq 4.5).$<br>
