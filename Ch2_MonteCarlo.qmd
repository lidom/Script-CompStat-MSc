...
$$
\mathbb{E}_{f_X}\left(h(X)\right)=\mathbb{E}\left(h(X)\right)=\int_\mathcal{X}h(x)\,f(x)\,dx,
$$
where $\mathcal{X}$ denotes the domain of the random variable $X\in\mathcal{X}\subseteq\mathbb{R}^d,$ and where $h$ is some transformation function (e.g. $h(x)=x^2,$ $h(x)=\ln(x),$ $h(x)=x,$ etc.). 


**Convergence:**

Given our previous developments, it is natural to propose using a realization $x_1,\dots,x_m$ from a (pseudo random) i.i.d. sample $X_1,\dots,X_m$ with each $X_j$ distributed as $X\sim f_X$ to approximate the above integral by the empirical mean
$$
\mathbb{E}\left(h(X)\right)\approx\bar{h}_m=\frac{1}{m}\sum_{j=1}^m h(x_j).
$$
By the [Strong Law of Large Numbers](http://www.statlect.com/asylln1.htm) we know that the empirical mean $\bar{h}_m$ converges almost surely (a.s.) to the desired limit $\mathbb{E}\left(h(X)\right)$ as the sample size $m$ becomes large, i.e., as $m\to\infty$. (The only prerequisits are that $h(X)$ has finite first moments, i.e., $\mathbb{E}\left(h(X)\right)<\infty$, and that $\bar{h}_m$ is constructed from an i.i.d. sample $X_1,\dots,X_m$.)
