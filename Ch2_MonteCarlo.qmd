...
<!-- ToDo: Rewrite Importance Sampling according to Wasserman All of Statistics 
...
@fig-MCIntPublishedExample shows a screenshot of a published example (see @Bourreau2021market). The authors use Monte Carlo integration to solve the shown integral. 


![Market share function in @Bourreau2021market. The authors use Monte Carlo integration to solve this integral.](images/MCIntPublishedExample2.png){#fig-MCIntPublishedExample}
...
## Classical Monte Carlo Integration using Proportions


Many integration problems, such as the one in @exm-ApproxNormTables, can be written as a proportion $0\leq p\leq 1.$ This allows us compute the integral using using empirical proportions 
$$
...
A possible way to construct normal distribution tables, i.e., table for the values of the distribution function $0\leq \Phi(x)\leq 1,$ is to use Monte Carlo integration. 
...
@tbl-CIProp gives the evolution of the Monte Carlo integration results for @exm-ApproxNormTables
$$
...
As in the case of classical Monte Carlo integration (see @eq-MeanIntegral), the generic problem is the evaluation of the integral 
$$
...
# [Cauchy](https://en.wikipedia.org/wiki/Cauchy_distribution) Tail Probability
...
$${#eq-CauchIntegral}
where $1/(\pi(1+x^2))$ is the density function of the $\mathcal{Cauchy}(0,1)$-distribution. 
...
@thm-MinVarTheorem gives us the best choice for the **importance function** $g.$
...
The **importance function** $g$ which minimizes the variance $\mathbb{V}_g(\psi(X_i))$, and, therefore, minimizes also the variance of the integral estimator in @eq-ImportSample, $\mathbb{V}_g(\hat{\theta}_g)$, is given by 
$$
...
\end{align*}
$${#eq-VarExpression}
Thus, the second term, $\left(\mathbb{E}_g(\psi(X_i))\right)^2,$ does not depend on $g.$ 
...
::: {.callout-tip}
**[Jensen's Inequality](https://www.statlect.com/fundamentals-of-probability/Jensen-inequality):** Let $u$ be a "convex function" such as, for instance, $u(x)=x^2,$ and let $X$ be some random variable. Then, 
$$
...
\end{align*}
$$
This shows that the lower bound of $\mathbb{E}_g(\psi(X_i)^2)$ is achieved for $g(x)=g^\ast(x)=\frac{|h(x)|f(x)}{\int |h(z)|f(z)dz},$ which implies the statement of @thm-MinVarTheorem. 

::: {.callout-note}
Note that @thm-MinVarTheorem may appear as **rather impractical**. If, for instance, $h(x)\geq 0$ then $g^\ast$ requires us to know $\theta=\int h(z)f(z)dz,$ which, however, is just the integral of interest! 

The minimum variance result of @thm-MinVarTheorem is, nevertheless, useful:
...
*  Furthermore, $g(x)$ should be chosen such that it has a thicker tail than $f(x)$, since the variance $\mathbb{V}(\hat{\theta}_g)$ crucially depends on the quotient $f(x)/g(x)$ (see @eq-VarExpression) which would "explode" for $g(x)\approx 0$.
:::
...
## Cauchy Tail Probability (continued) 

Above, in @exm-CauchTailProb, we had
$$
\begin{align*}
p
&=\int_{2}^{+\infty}\frac{1}{\pi(1+x^2)}\,dx\\[2ex]
...
This choice takes into account our insights from @thm-MinVarTheorem, since 
...
#| eval: true
#| label: fig-CauchyExm
#| fig-cap: Density function $f(x)=1/(\pi(1+x^2)),$ and importance function $g(x)=2/x^2$ for $x\geq 2.$
# g(x):
g_fun <- function(x){2/(x^2)}
# 
xx    <- seq(from= 2, to=15, length.out = 500)
# plot
...
abline(v=2, lty=2)
# legend
legend("topright", legend = c("Cauchy(0,1)", expression(g(x)==2/x^2)), lty=c(1,1),
...
F1 <- antiD( 2 * (1/(2 * 3.14159265358979 * (1+u^2)))^2 ~ u )
F2 <- antiD( 2 * (1/(2 * 3.14159265358979 * (1+u^2)))   ~ u )

F1(1/2) - F1(0)  - (F2(1/2) - F2(0))^2
...
<!-- Example 6.19 p. 178, A First Course in Statistical Programming with R (2021) -->
...
<!-- Example 6.20 p. 178, A First Course in Statistical Programming with R (2021) -->
...
<!-- Tail probability example of Wasserman All of Statistics 
25.3 Importance Sampling -->
...
\theta = P(X\geq 3),
$$
where $X\sim\mathcal{0,1},$ using Monte Carlo integration. 

By the way, the true value is about this one: 
```{r}
round(pnorm(3, mean = 0, sd = 1, lower.tail = FALSE), 5)
```

(a) Write the parameter $\theta$ as 
$$
...
(b) What is the classical Monte Carlo integration estimator, $\hat{\theta},$ of $\theta$? Compute one estimate of $\theta$ using a sample size of $n=100.$

(c) Repeat the simulation many times (e.g. $10,000$ times) to get an estimate of the mean, $\mathbb{E}(\hat{\theta}),$ and the variance, $\mathbb{V}(\hat{\theta}),$ of $\hat{\theta}.$  

(d) Why is the classic Monte Carlo integration approach here not very efficient? 

(e) Take now an importance sampling approach and write the parameter $\theta$ as 
$$
\theta = \int \frac{h(x)f(x)}{g(x)} g(x) dx = \mathbb{E}_g(Y),
$$
where $Y=\frac{h(X)f(X)}{g(X)}$ and $X\sim g.$ <br> Why is the density function $g\sim\mathcal{N}(4,1)$ potentially a good choice? 

(f) What is the importance sampling estimator, $\hat{\theta}_{IS},$ of $\theta$? Compute one estimate of $\theta$ using a sample size of $n=100.$ 

(g) Repeat the simulation many times (e.g. $10,000$ times) to get an estimate of the mean, $\mathbb{E}(\hat{\theta}),$ and the variance, $\mathbb{V}(\hat{\theta}),$ of $\hat{\theta}.$  


#### Exercise 4. {-} 

The Bavarian city [Passau](http://regiowiki.pnp.de/images/Hochwasser-Luftbild-Binder.jpg) needs to know tomorrows probability of facing a [flood catastrophy](http://regiowiki.pnp.de/images/Wasserstand_Rathausturm_2014.jpg). They received a density function $f$ from some experts which describes tomorrows distribution of standardized water levels $X$. The city is particularly interested in the probability of observing water levels above of 11 meters, which in standardized water levels is equivalent to the event $X\geq 4.5$, where $X\sim f$. 

Unfortunately, $f$ is a very complicated function such that they cannot compute the probability analytically. Therefore, they tried to approximate the probability $P(X\geq 4.5)$ using classical Monte Carlo integration.<br>

(a) Derive the classical Monte Carlo integration estimator.<br>
**Hint:** You can assume that it is possible to sample from $f.$ (This is not a restrictive assumption, since one could use, for instance, the very general  Accept-Reject-Algorithm.)

(b) Show that the classical Monte Carlo integration estimator is unbiased. 

(c) Derive the variance of the classical Monte Carlo integration estimator.  

(d) It turns out that the tail probabilities of $f$ are *very* low and when simulating, e.g., $B=10,000$ realizations from $f$ usually no single realization is greater or equal to $4.5$.<br>
Derive the importance sampling estimator using an appropriately shifted version of the exponential density function with $\lambda=1$ as the importance function $g$ such that all of the $B=10,000$ realizations are used for the estimation of the probability $P(X\geq 4.5).$  

(e) Show that the importance sampling estimator is unbiased. 

(f) Show (mathematically) that the variance of the importance sampling estimator of the probability $P(X\geq 4.5)$ is strictly smaller than the variance of the classical Monte Carlo integration estimator.<br>
**Hint:** You can assume and should use that $g(x) > f(x)$ for all $x\geq 4.5.$ 
