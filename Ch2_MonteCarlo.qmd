...
$$
\begin{align*}
\mathbb{V}\left(n\hat{p}_n(x)\right)&=n\Phi(x)(1-\Phi(x)).
...
\end{align*}
$$
i.e., here $f$ is the density of the $\mathcal{Cauchy}(0,1)$-distribution and $h(x)=1_{(x\geq 2)}$. 
...
Applying Jensen's inequality yields, the following lower bound for $\mathbb{E}_g(\psi(X_i)^2):$
$$
\begin{align*}
\mathbb{E}_g(\psi(X_i)^2)
