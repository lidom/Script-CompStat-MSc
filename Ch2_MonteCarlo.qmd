...
Using Jensen's inequality, we can derive the lower bound for $\mathbb{E}_g(\psi(X_i)^2):$
$$
\begin{align*}
\mathbb{E}_g( \; (\psi(X_i) ) ^2 \; )
...
\end{align*}
$$
This shows that the lower bound of $\mathbb{E}_g(\psi(X_i)^2)$ is achieved for $g(x)=g^\ast(x)=\frac{|h(x)|f(x)}{\int |h(z)|f(z)dz},$ which shows the statement of @thm-MinVarTheorem. 

::: {.callout-note}
Note that @thm-MinVarTheorem may appear as rather impractical, since, for instance, if $h(x)\geq 0$ then $g^\ast$ requires us to know $\theta=\int h(z)f(z)dz,$ which, however, is just the integral of interest! 

The minimum variance result of @thm-MinVarTheorem is, however, still useful:
...
\end{align*}
$$
where $f$ is the density of the $\mathcal{Cauchy}(0,1)$-distribution and $h(x)=1_{(x\geq 2)}$. 
...
A possibly good (and simple) choice of $g$ is, for instance, 
$$
g(x)=\left\{
  \begin{array}{ll}
  \frac{2}{x^2}& x\geq 2\\
...
:::{.callout-note}
The function $g$ is indeed a density function, since it is non-negative and integrates to one.
...
0 - F(2) 
```
(Double-check this also without computer support.)
:::
...
\end{align*}
$$ 
An integration by parts shows that $\mathbb{V}(\hat{p}_4)=0.95\cdot 10^{-4}/n$. Compare this to the former results: $\mathbb{V}(\hat p_3)=0.0285/n$, $\mathbb{V}(\hat{p}_{2})=0.0525/n$ and $\mathbb{V}(\hat{p}_{1})=0.1275/n$. The variance of $\hat{p}_4$ is by a factor of $10^{-3}$ lower than the variance of the original $\hat{p}_1$.
