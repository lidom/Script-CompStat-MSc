...
\begin{align*}
F_n(x) 
& = \frac{1}{n} 1_{(X_i\leq x)}\\
\Rightarrow nF_n(x) 
& = 1_{(X_i\leq x)} \sim \mathcal{Binom}\left(n,p=F(x)\right),
\end{align*}
$$
since $1_{(X_i\leq x)}$ is a Bernoulli random variable with parameter 
$$
p=P(1_{(X_i\leq x)} = 1) = P(X_i \leq x) = F(x).
$$

##### (a) Part 2. {-}

The asymptotic  point-wise distribution of $F_n(x)$ for a given $x\in\mathbb{R}.$  From part 1, we have that 
$$
\begin{align*}
...
Moreover, since $F_n(x)  = \frac{1}{n} 1_{(X_i\leq x)}$ is an average over i.i.d. random variables $1_{(X_1\leq x)},\dots,1_{(X_n\leq x)},$ the standard CLT implies 
$$
...
I.e., if it is possible to draw an $\varepsilon$-band around the graph of $g(x)$ that contains all of the graphs of $g_n(x)$ for large enough $n.$
::: 
...
For Non-Normal distributions $F$ we have by the Central [Limit Theorem](https://www.statlect.com/asymptotic-theory/central-limit-theorem):
$$
\begin{array}{rlc}
...
$$
\mathbb{P}\left(\mu\in \operatorname{CI}_{\operatorname{classic},n}\right)\to 1-\alpha\quad\text{as}\quad n\to\infty.
$$

Let us consider the finite-$n$ (with $n=30$) performance of the classic confidence interval for the case where $F$ is a **normal distribution** with mean $\mu=1$ and standard deviation $\sigma=2$:
```{r}
##  Setup:
n     <-   30 # Sample Size
mean  <-    1 # Mean
...
##### (a) Part 2: Classic Confidence Interval ($n=30$ and $X_i\sim \chi^2_1$) {-}
...
##### (b) Standard bootstrap confidence interval ($n=30$ and $X_i\sim \chi^2_1$) {-}
...
##### (c) Bootstrap-$t$ confidence interval ($n=30$ and $X_i\sim \chi^2_1$) {-}
...
In the following we generate the bootrap distribution of $(\bar{X}_n^\ast-\bar{X}_n)/\hat\sigma^\ast$, compute the quantiles $\hat{\tau}_{\alpha/2}$ and $\hat{\tau}_{1-\alpha/2}$, and plot all of this:
