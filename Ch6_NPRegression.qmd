...
$$ 
is a lest squares solution; namely of the following least squares problem that tries to approximate the unknown $m$ using a spline function $s$:
$$
\begin{align*}
...
$$
as the number of knots $p\to\infty.$ 
:::
...
&=\frac{1}{n} \sigma^2 \text{trace}\left(I_n\right)\\[2ex]
...
* $p\to\infty$ as $n\to\infty$ such that $\dfrac{p}{n}\to\infty$
...
An **optimal smoothing parameter** $p,$ balancing bias and variance, will be of order 
$$
\begin{align}
