...
2. Assign the data points $x_i$ to the group that maximizes the **"posterior probability"** (@fig-GMM-plot1)
...
The following `R` codes can be used to replicate the above cluster analysis and @fig-GMM-plot1:
...
😒 **Problems with singularities in numerical solutions:** If one tries to solve the above maximization problem [numerically with the help of the computer](https://cran.r-project.org/web/packages/EstimationTools/vignettes/maxlogL.pdf), one will quickly notice that the results are highly unstable, implausible and not very trustworthy. The reason for these unstable estimates are problems with singularities. 

For real GMMs ($G>1$), problems with singularities occur very easily during a numerical maximization. This happens whenever one of the normal distribution components tries to describe the whole data set $\mathbf{x}$ and the other(s) normal distribution component(s) try to describe only single data points. A Gaussian density function centered around a single data point $x_i,$ i.e. 
$$
\mu_g=x_i\quad\text{and}\quad  \sigma_g\to 0
$$ 
will thereby assume very large values, 
$$
f(x_i|\mu_g=x_i,\sigma_g)\to\infty\quad\text{for}\quad \sigma_g\to 0
$$ 
and thus maximize the log-likelihood in an undesirable way (see @fig-dirac1). Such **undesirable, trivial maximization solutions** usually lead to implausible estimation results.
...
$$
for $i=1,\dots,n$ and $g=1,\dots,G$. <br>
...
## The Alternative / True View on the EM Algorithm
...
If we know the flipper length of penguin $i$ then we can update the prior probability using **Bayes' Theorem** which leads to the posterior probability: <br>
...
If, in addition to the data points, $\mathbf{x}=(x_1,\dots,x_n),$ we had also observed the group assignments, $\mathbf{z}=(z_{11},\dots,z_{nG}),$ then we could establish the following **likelihood ($\tilde{\mathcal{L}}$) or log-likelihood ($\tilde{\ell}$) function**:
$$
\begin{align*}
...
The average penguin does not care about the EM Algorithm.
