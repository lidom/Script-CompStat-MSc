...
X_1,\dots,X_n,
$$
it's called a **unsupervised classification** (computer science: unsupervised learning) or clustering problem.


### Bayes Classifier 

We would like to find a classification rule $h$ that makes accurate
predictions. To measure accuracy, we need a accuracy-criterion. The most often used one is the **error rate**.
...
L(h) = P(h(X)\neq Y).
$$ 
The **empirical/training error rate** is
$$
