...
3. Prüfe Konvergenz: 
    ...
    - Stoppe, falls sich der Wert der maximierten Log-Likelihood Funktion, $\ell(\boldsymbol{\pi}^{(r)},\boldsymbol{\mu}^{(r)},\boldsymbol{\sigma}^{(r)}|\mathbf{x})$, nicht mehr ändert.


<br>

The above pseudo code is implemented in the following code chunck:
...
results <- matrix(c(pi, mu, sqrt(sigma)), 
                  ...
                  dimnames = list(
            c("weights", "means", "standard-deviations"),
            c("group 1", "group 2"))) 
##
results %>% round(., 2)
```
