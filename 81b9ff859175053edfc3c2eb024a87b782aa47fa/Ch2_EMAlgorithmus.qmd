...
🤓 **Analytic solution:** It is a bit tedious, but one can maximize the log-likelihood analytically. If you do this, you will get the following expressions:
$$
\begin{align*}
...
::: {.callout-note}
Still, we cannot properly define what we mean by "prior probability" and "posterior probability". We'll discuss the prior and the posterior probability in more detail in @sec-PriorPosterior.
:::
...
2. For $r=1,2,\dots$
    ...
    - <span style="color:#FF5733">**(Expectation)**</span> Compute: 
     $$p_{ig}^{(r-1)}=\frac{\pi_g^{(r-1)}\varphi(x_i|\mu^{(r-1)}_g,\sigma_g^{(r-1)})}{f_{GMM}(x_i|\boldsymbol{\pi}^{(r-1)},\boldsymbol{\mu}^{(r-1)},\boldsymbol{\sigma}^{(r-1)})}$$
...
Unfortunately, the true group labels $z_{ig}$ are unknown. However, we nevertheless know something about these assignments. The weights 
$$
\pi_1,\dots,\pi_G
$$ 
...
f_{GMM}(x|\boldsymbol{\pi},\boldsymbol{\mu},\boldsymbol{\sigma})=\sum_{g=1}^G\pi_g\varphi(x|\mu_g,\sigma_g),
$$
give us the proportions of the individual distributions $\varphi(\cdot|\mu_g,\sigma_g)$ in the total distribution $f_{GMM}$. 
...
Thus, we can consider the unobserved group label $z_{ig}$ as a (unobserved) realization of a binary random variable $Z_{ig}\in\{0,1\}$ with probability mass function
$$
\begin{align*}
P(Z_{ig}=1)&=\pi_g\\[2ex]
...
If, in addition to the data points, $\mathbf{x}=(x_1,\dots,x_n),$ we had also observed the group assignments, $\mathbf{z}=(z_{11},\dots,z_{nG}),$ then we could establish the following alternative **likelihood ($\tilde{\mathcal{L}}$)** and **log-likelihood ($\tilde{\ell}$) functions**:
$$
\begin{align*}
...
Unlike the original log-likelihood function (@eq-logLikGMM), the new log-likelihood function $\tilde\ell$ would be **easy to maximize**: we can directly calculate the logarithm of the normal distribution.  This simplifies the maximization problem considerably, since the normal distribution belongs to the exponential family (see @sec-MLAsymp) which is not the case for the normal mixture distribution.
...
The problem of predicting a discrete random variable $Y$ (i.e. the group labels) from a possibly multivariate random variable $X$ is called **classification**. 

**Example:** $Y\in\{0,1\}$ and $X\in\mathbb{R}^p$
...
\end{array}\right.
$$
Example: Linear/quadratic discriminant analysis, naive Bayes, Gaussian mixture distributions
...
$$
where $\pi_g$ is the **prior probability**
$$
\pi_g = P(Y=g),
$$
...
(e) Derive the expectation of $\tilde\ell,$ $\mathbb{E}_{\mathbf{p},\boldsymbol{\pi}}\left(\tilde{\ell}(\mathbf{p},\boldsymbol{\pi}|\mathbf{x},\mathbf{Z})\right)$.

(f) Maximize $\mathbb{E}_{\mathbf{p},\boldsymbol{\pi}}\left(\tilde{\ell}(\mathbf{p},\boldsymbol{\pi}|\mathbf{x},\mathbf{Z})\right)$ with respect to $p_g$ for $g=1,\dots,G.$

(g) Maximize $\mathbb{E}_{\mathbf{p},\boldsymbol{\pi}}\left(\tilde{\ell}(\mathbf{p},\boldsymbol{\pi}|\mathbf{x},\mathbf{Z})\right)$ with respect to $\pi_g$ for $g=1,\dots,G$ such that $\sum_{g=1}^G\pi_g=1.$

(h) Sketch the EM-Algorithm


<!-- {{< include Ch2_EMAlgorithmus_Solutions.qmd >}} -->
