...
because the probabilities $0\leq p_{ig,0}\leq 1$ (defined in @eq-AnalyticSol2) depend on these unknown parameters. 
...
$$ 
in @eq-AnalyticSol2 is called the **posterior probability**. The posterior probability $p_{ig,0}$ is the probability that penguin $i$ with flipper length $X_{i}$ belongs to group $g.$ 
...
$$
using $\hat{\pi}_g$, $\hat{\mu}_g$, $\hat{\sigma}_g$ defined in @eq-AnalyticSol1 using estimates $\hat{p}_{ig}$ for the unknown $p_{ig,0}.$
...
\end{align*}
$${#eq-posteriorMean}
Thus, the computation of $p_{ig,0}$ is the **<span style="color:#FF5733">**E**</span>xpectation**-step of the EM algorithm (@sec-EM1). 
::: 
...
**Idea:** Substitute the unobservable $Z_{ig},$ by their best (in the mean square sense) prediction $\hat{Z}_{ig,0}$; namely, the conditional mean (using @eq-posteriorGMD and @eq-posteriorMean)
$$
\begin{align*}
...
> **Note:** Above in @sec-TrueViewEM, we used the dummy variables $Z_{ig},$ to encode the discrete (categorial) $Y$ group labels.
...
$$ 
Thus, the realization $Z_i=(0,1,0,\dots,0)$ means that the $i$th observation comes from the $2$nd Bernoulli distribution $\text{Bernoulli}(p_{2,0})$.</br> 
Write the expressions of the (log) likelihood functions $\tilde{\mathcal{L}}(\mathbf{p},\boldsymbol{\pi})$ and $\tilde{\ell}(\mathbf{p},\boldsymbol{\pi})$ that take into account the latend group indicator random variables.  
...
(e) Derive the conditional expectation of $\tilde\ell,$ given $X_1,\dots,X_n,$ 
$$\mathbb{E}_{\mathbf{p}_0,\boldsymbol{\pi}_0}\left(\tilde{\ell}(\mathbf{p},\boldsymbol{\pi})\big|X_1,\dots,X_n\right).
$$

(f) Maximize $\mathbb{E}_{\mathbf{p}_0,\boldsymbol{\pi}_0}\left(\tilde{\ell}(\mathbf{p},\boldsymbol{\pi})\big|X_1,\dots,X_n\right)$ with respect to $p_g$ for $g=1,\dots,G.$

(g) Maximize $\mathbb{E}_{\mathbf{p}_0,\boldsymbol{\pi}_0}\left(\tilde{\ell}(\mathbf{p},\boldsymbol{\pi})\big|X_1,\dots,X_n\right)$ with respect to $\pi_g$ for $g=1,\dots,G$ such that $\sum_{g=1}^G\pi_g=1.$
...
1. We only need to focus on the $\pi_g$ terms since the other terms in  
$$
\begin{align*}
&\mathbb{E}_{\mathbf{p}_{0},\boldsymbol{\pi}_{0}}\left(\tilde\ell(\mathbf{p},\boldsymbol{\pi})\mid X_1,\dots,X_n\right)\\[2ex] 
& = \sum_{i=1}^n \sum_{g=1}^G \mathfrak{p}_{ig,0} \left(\ln(\pi_g) + \ln\left(p_g^{X_i}(1-p_g)^{1-X_i}\right)\right)
...
2. However, we need to do maximization under the side constraint that $\sum_{g=1}^G\pi_g=1.$ So, we use the method of Lagrange multipliers. 
...
3. Check convergence (e.g., no relevant change of the maximized log-likelihood function). 
