...
results           <- cbind(theta, -Lp/Lpp, Lp)
colnames(results) <- c("theta_m", "h", "Lp(theta_m)")
results
```
...
\end{align*}
$$
for some real number $\xi_{(m)}$ between $\theta_{root}$ and $\theta_{(m)}.$ 
...
For $0<M\,|e_{(0)}|<1$ the inequality in @eq-NRIneq becomes a sharp inequality
$$
\begin{align*}
&|e_{(1)}|\leq \overbrace{M\;|e_{(0)}|}^{<1}\,|e_{(0)}|
\quad\Rightarrow\quad |e_{(1)}|< \,|e_{(0)}|\\[2ex]
\Rightarrow\quad 
& |e_{(2)}|\leq \overbrace{M\;|e_{(1)}|}^{<1}\,|e_{(1)}|
...
\end{align*}
$$ 
even when $|e_{(0)}|\gg 0.$ This makes sense, since $\sup_{\theta\in I}|\ell'''(\theta)|=0$ implies that $\ell'(\theta)$ has no curvature; i.e. $\ell'(\theta)$ is a straight line for all $x\in I$ which implies that the Taylor approximation used in the update steps of the Newton-Raphson algorithm is just perfect (no approximation error).
