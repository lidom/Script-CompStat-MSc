...
The basis functions $b_1,b_2,\dots$ have to be chosen in such a way that for *any possible* smooth function $m$ the squared approximation error 
$$
\min_{\beta_1,\dots,\beta_p} \left(m(x)-\sum_{j=1}^p \beta_j b_j(x)\right)^2
$$ 
...
Therefore, $\hat{m}_p(x)$ is typically a biased estimator for given values of $p,$ 
$$
\begin{align*}
...
the point-wise at each $x$-value in `x_vec`.
...
Pt_BiasSq_p3 <- Pt_Bias_p3^2
Pt_BiasSq_p6 <- Pt_Bias_p6^2
...
par(mfrow=c(1,2))
matplot(y = cbind(Pt_BiasSq_p1, Pt_BiasSq_p3, Pt_BiasSq_p6), 
...
$$
\tau_{j+1}-\tau_j=\tau_j-\tau_{j-1}\quad\text{for all } j=2,\dots,q-1.
$$
...
$$ 
for splines of degree $k$ based on a knot sequence 
$$
a=\tau_1<\tau_2\leq\dots\leq \tau_{q-1}<\tau_q=b
$$ 
...
$${#eq-BSplineRecursion}
$j=1,\dots,q+k-1$, and $x\in [a,b],$ where 

where $\tau_j^\ast$ denotes the augmented knot sequence: 
...
Note: @eq-BSplineRecursion is understood that if the denominator is 0, then the function is defined to be 0. 
...
$$ 
cubic ($k=3$) B-spline basis functions. Let's take a look at them:
...
all_knots      <- sort(c(rep(boundary_knots[1], order), 
                         internal_knots, 
                         rep(boundary_knots[2], order)))
...
::: {.callout-note}
The B-spline basis system has a property that is often useful: the sum of the B-spline basis function values at any point $x$ is equal to one. Note, for example, that the first and last basis functions are exactly one at the boundaries. This is because all the other basis functions go to zero at these end points. 

Basis functions are positive only over at most $k+1$ intervals and zero over the remaining intervals. This compact support property is important for computational efficiency since the effort required is proportional to $p$ as a consequence, rather than to $p^2$ for basis functions not having this local support property.
:::
...
$$
\hat\beta_1,\dots,\hat\beta_p=\arg\min_{\beta_1,\dots,\beta_p} \sum_{i=1}^n \left( Y_i-\sum_{j=1}^p \beta_j \underbrace{b_{j,k}(X_i)}_{X_{ij}}\right)^2
$$
...
$$ 
is referred to as the **smoothing matrix** and the number of B-spline basis function $p$ is referred to as the **smoothing parameter**. 
...
$$
\begin{align*}
\sum_{i=1}^n \left(m(X_i)-\sum_{j=1}^p \beta_j b_{j,k}(X_i)\right)^2 
&=\min_{\vartheta_1,\dots,\vartheta_p}\sum_{i=1}^n \left(m(X_i)-\sum_{j=1}^p \vartheta_j  b_{j,k}(X_i)\right)^2\\
...
$$
is the best ($L_2$) approximation of the true, but unknown, regression function vector $m=(m(X_1),\dots,m(X_n))^\top$ by means of a spline function vector $s=(s(X_1),\dots,s(X_n))^\top$ with $s\in{\cal{S}}_{k,\tau_1,\dots,\tau_q}.$
...
$$
as $n\to\infty$ and $p\equiv p_n\to\infty.$ 
:::
...
&=\frac{1}{n}\mathbb{E}_\varepsilon\left(\left\Vert \left(\begin{matrix}\hat{m}_p(X_1)\\\vdots\\\hat{m}_p(X_n)\end{matrix}\right)-\left(\begin{matrix}\tilde{m}_p(X_1)\\\vdots\\\tilde{m}_p(X_n)\end{matrix}\right)\right\Vert_2^2\right)\\[2ex]
&=\frac{1}{n}\mathbb{E}_\varepsilon\left(\Vert \mathbf{X}(\mathbf{X}^\top \mathbf{X})^{-1}\mathbf{X}^\top Y-
...
From @eq-BiasSqVar we have that 
$$
\begin{align*}
...
**Problem:** 
Since $m$ is unknown, we cannot directly compute $\operatorname{MASE}(\hat{m}_p)$ and thus cannot compute the exact value of $p_{opt}$. However, we need to choose the smoothing parameter $p$ in an (somehow) optimal and objective manner.
...
* For any $p$ the error criterion provides "information" about the respective  $\operatorname{MASE}(\hat{m}_p)$.
