...
In nonparametric regression analysis, we do not make assumptions about the structure of the regression function $m(x).$ We only make the qualitative assumption that $m$ is a sufficiently **smooth** function, i.e. that $m(t)$ is sufficiently often differentiable at every $t\in(a,b)$.
:::
...
Examples are approximations by: 
...
Let's look at the fits of 200 Monte Carlo replications: 
