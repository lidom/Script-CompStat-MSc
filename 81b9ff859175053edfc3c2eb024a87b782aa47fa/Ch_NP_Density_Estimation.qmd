...
@fig-HistKDE shows a comparison of the histogram versus the density estimate from a kernel density estimatior. 
...
$1_{(\text{TRUE})}=1$ and $1_{(\text{FALSE})}=0.$ This guarantees that the bin areas of the histogram sum up to one
$$
\begin{align*}
\sum_{j=1}^J\text{Area of $j$th Bin}
&=\sum_{j=1}^J \frac{1}{n}\sum_{i=1}^n \; 1_{(X_i\in(x_{j-1},x_j])}\\[2ex]
&=\frac{1}{n} \sum_{j=1}^J \sum_{i=1}^n \; 1_{(X_i\in(x_{j-1},x_j])}=\frac{n}{n}=1.
\end{align*}
$$


Doing some smart rearrangements of @eq-fHist, allows writing the histogram as a type of kernel density estimator:  
$$ 
\begin{align*}
\hat f_{hist}(\bar{x}_j)
