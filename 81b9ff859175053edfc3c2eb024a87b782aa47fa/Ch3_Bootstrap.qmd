...
$$
Thus, for doing inference (confidence intervals, statistical tests, etc.) about $\mu_0,$ we actually use the estimated distribution 
$$
...
$$
\begin{align*}
\bar{X}_n&\overset{a}{\sim}\mathcal{N}\left(\mu_0,\frac{\hat{\sigma}_n^2}{n}\right)\quad\Leftrightarrow\quad \bar{X}_n\overset{a}{\sim}H^{Asymp}_n
\end{align*}
$$
for computing, for instance, an estimator for the standard error $\operatorname{SE}\big(\bar{X}_n\big),$ 
$$
\widehat{\operatorname{SE}}^{Asymp}_{n}\big(\bar{X}_n\big)=\sqrt{\frac{\hat{v}^2}{n}}=\sqrt{\frac{\hat\sigma_n}{n}}.
$${#eq-SEAsymp}
...
$$ 
is a legit alternative to $H^{Asymp}_n(x).$ I.e. we check whether we can use $H^{Boot}_{n}(x)$ as an alternative for estimating the limit distribution $\Phi_{\sigma_0}(x).$ 
...
$$
then we can use $H^{Boot}_{n,m}(x)$ (instead of $H^{Asymp}_{n}(x)$) to do inference about $\mu_0.$ 

The following `R`-code computes the following distribution functions
...
\end{align*}
$$
can be computed **directly** from $\bar{X}^*_{n,1},\dots,\bar{X}^*_{n,m}.$ There's no need for deriving a formula $v^2=\sigma_0^2$ for the asymptotic variance as in @exm-AsympNorm for $H^{Asymp}_{n}(x).$  

In fact, we can derive an estimator for the asymptotic variance $v^2=\lim_{n\to\infty}\sqrt{n}\big(\bar{X}_n-\mu_0\big)$ directly from the Bootstrap distribution $H^{Boot}_n(x)\colon$  
$$
\begin{align*}
\hat{v}^2_{Boot,n}
...
$$ 
can be computed *directly* from $\bar{X}^*_{n,1},\dots,\bar{X}^*_{n,m}.$ For our artifical data example, we have that 
<center>
$\hat{v}^2_{Boot,n}=$ `n * var(Xbar_boot_vec)` $=$ `r round(n * var(Xbar_boot_vec), 2)`
</center>
</br>

There's no need for deriving a formula $v^2=\sigma_0^2$ for the asymptotic variance as in @exm-AsympNorm.  
...
::: {.callout-note}
Computing an Bootstrap estimate of the standard error of an estimator is typically that simple: Compute the standard deviation of $\hat\theta^*_{n,1},\dots,\hat\theta^*_{n,m}$ for a large $m.$ 
:::
