...
Since we can choose $m$ arbitrarily large, we can effectively ignore the approximation error between 
$$
H^{Boot}_{n,m}(x)\quad\text{and}\quad H^{Boot}_{n}(x).
$$ 
That is, we can (and will do so) treat the bootstrap distribution $H^{Boot}_{n}(x)$ **as known.** 🤓  
...
Here, it is rather simple to derive an useful expression for the asymptotic variance $v^2\colon$
$$
\begin{align*}
v^2
...
$$
Thus, for doing inference (computing the standard error, quantiles, etc.) about $\mu_0,$ we actually use  
$$
H^{Asymp}_n(x)=\Phi_{\hat\sigma_n}(x)=\Phi\left(\frac{x}{\hat\sigma_n}\right),
$$
where 
$$
H^{Asymp}_n(x)\to\Phi_{\sigma_0}(x)\quad\text{as}\quad n\to\infty
$$
for all continuity points $x.$

An estimator for the standard error $\operatorname{SE}\left(\bar{X}_n\right)$ is given by
$$
\widehat{\operatorname{SE}}^{Asymp}_{n}\left(\bar{X}_n\right)=\sqrt{\frac{\hat{v}^2}{n}}=\sqrt{\frac{\hat\sigma_n}{n}}.
$$
...
$$ 
is a legit alternative to $H^{Asymp}_n(x);$ i.e. whether we can use $H^{Boot}_{n}(x)$ to approximate the limit distribution $\Phi_{\sigma_0}(x).$ 

Before we answer this question theoretically (see @sec-Theory1 and @sec-Theory2BootstrapConsist), we check it empirically using an **artifical data example.** 
...
* True (usually unknown) asymptotic limit distribution: 
  $$
  \begin{align*}
  \sqrt{n}(\bar{X}_n-\mu_0)&\to_d\mathcal{N}(0,4)\\[2ex]
  \Leftrightarrow\quad H_n(x)&\to_p \Phi_{2}(x)\quad\text{for all continuity points } x.
  \end{align*}
  $$ 
...
$$
by sampling (i.i.d.) $n=100$ data points from a $\chi^2$-distribution with $\operatorname{df}=2.$ The first six data points are shown in @tbl-ChiSqSample. 
...
<center>
$\bar X_{n} =$ `mean(observedSample)` $=$ `r mean(observedSample)`
</center>
...
Each realization of the bootstrap sample leads to a new realization of the bootstrap estimator $\bar{X}^*_n.$ 
...
```{r}
m                <- 10000 # number of bootstrap replications 
...
$$
Thus, by choosing a large $m$ (e.g. $m=10000$) we can proceed as if 
$$
...
$$
then we can directly use $H^{Boot}_{n,m}(x)$ (instead of $H^{Asymp}_{n}(x)$) to do inference about $\mu_0.$ 

The following `R`-code computes 
...
and plots its graph against the graph of $\Phi_{2}(x)\colon$
...
\end{align*}
$$
can be computed *directly* from the bootstrap realizations of the estimator $\bar{X}^*_{n,1},\dots,\bar{X}^*_{n,m}.$ All we need to know is how to compute the estimators. 

From the Bootstrap distribution $H^{Boot}_n(x)$ we can derive an estimator for the asymptotic variance $v^2=\lim_{n\to\infty}\sqrt{n}\left(\bar{X}_n-\mu_0\right)\colon$  
...
&=n \; Var\left(\left.\bar{X}^*_n\;\right|\;\mathcal{S}_n\right)\\[2ex]
&\overset{m\to\infty}{=} n \;\cdot\;\underbrace{\frac{1}{m}\sum_{j=1}^m \left(\bar{X}^*_{n,j} - \left(\frac{1}{m}\sum_{j=1}^m\bar{X}^*_{n,j}\right)\right)^2}_{=\widehat{Var}_m\left(\left.\bar{X}^*_n\;\right|\;\mathcal{S}_n\right)}\\[2ex]
\end{align*}
$$

Thus, estimator $\hat{v}^2_{Boot,n}$ can be computed *directly* from the bootstrap realizations of the estimator $\bar{X}^*_{n,1},\dots,\bar{X}^*_{n,m}.$  
...
By constrast, when using the classic asymptotic statistics approach (see @exm-AsympNorm)


 to compute $H^{Asymp}_{n}(x),$ we (i) need to know that the asymptotic variance $v^2=\sigma_0^2,$  and (ii) need an estimator $\hat{v}^2=\hat\sigma_n^2$ for the asymptotic variance. Deriving the asymptotic variance and estimating it can be very hard (see @sec-Illustration).
