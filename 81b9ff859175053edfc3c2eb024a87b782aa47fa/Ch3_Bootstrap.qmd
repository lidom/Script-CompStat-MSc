...
* True (usually unknown) asymptotic limit distribution: 
  ...
  \Leftrightarrow\quad H_n(x)&\to_p \Phi_{2}(x)\quad\text{as}\quad n\to\infty,
  \end{align*}
  $$ 
...
H^{Boot}_n(x) = P\left(\left.\sqrt{n}\left(\bar{X}^*_n-\bar{X}_n\right)\;\right|\;\mathcal{S}_n\right),
$$
we can derive an estimator for the asymptotic variance $v^2=\lim_{n\to\infty}\sqrt{n}\big(\bar{X}_n-\mu_0\big)$ directly from $\bar{X}^*_{n,1},\dots,\bar{X}^*_{n,m}\colon$  
$$
\begin{align*}
\hat{v}^2_{Boot,n}
...
&[\text{Cond.~on $\mathcal{S}_n,$ $\bar{X}_n$ is a constant:}]\\[2ex]
