...
::: {.callout-note appearance="minimal"}
::: {#exm-AsympNorm}
# Recap: Inference using Classic Asymptotic Statistics 
...
$$
for all continuity points $x,$ where the asymptotic variance is here given by
$$
\begin{align*}
\sigma_0^2
...
Can we, alternatively to the classic asymptotic statistics appraoch in @exm-AsympNorm, use the Bootstrap? I.e., is 
$$
H^{Boot}_{n}(x)=P\left(\left.\sqrt{n}\left(\bar{X}^*_n-\bar{X}_n\right)\leq x\;\right|\;\mathcal{S}_n\right)
$$ 
able to approximate the limit distribution $\Phi_{\sigma_0}(x)$ for all $x\in\mathbb{R}$? 

Before we answer this question theoretically (see @sec-Theory1 and @sec-Theory2BootstrapConsist), we check it empirically using an **artifical** data example with 
$$
X_1,\dots,X_n\overset{\text{i.i.d.}}{\sim}X,
$$ 
where 

* $X\sim \chi^2_{\operatorname{df}},$ 
* $\operatorname{df}=2,$
* $\mu_0=\operatorname{df}=2,$ and
* $n=8.$   
...
X_1,\dots,X_n\overset{\text{i.i.d.}}{\sim}X,
$$ 
where $X\sim \chi^2_{\operatorname{df}}$ with $\operatorname{df}=2.$ 

**Question to be checked:** Is $H^{Boot}_{n}(x)$ able to approximate the asymptotic distribution the limit distribution $\Phi_{\sigma_0}$ of $\sqrt{n}(\bar{X}_n-\mu_0)$? If yes, we can directly work with $H^{Boot}_{n}(x)$ instead of working with $\Phi_{\sigma_0}.$

Since we know here that $X_1,\dots,X_n\overset{\text{i.i.d.}}{\sim}X$ with $X\sim \chi^2_{\operatorname{df}}$ and $\operatorname{df}=2,$ we know (see @exm-AsympNorm) that
$$
\begin{align*}
\sqrt{n}(\bar{X}_n-\mu_0)&\to_d\mathcal{N}(0,\sigma_0^2)\\[2ex]
\Leftrightarrow\quad H_n(x)&\to_p \Phi_{\sigma_0}(x)\quad\text{for all}\quad x,
\end{align*}
$$ 
where $\sigma_0^2 = Var(X) = 2\cdot \operatorname{df}=4.$ 

Let us consider the following **observed sample** $\mathcal{S}_n=\{X_1,\dots,X_n\}$ with a rather smallish sample size of $n=8$ shown in @tbl-ChiSqSample. The data was generated by drawing from a $\chi^2$-distribution with $\operatorname{df}=2.$  

> Choosing a smallish data size of $n=8$ is done to maintain a clear presentation. The approximation of $\Phi_{\sigma_0}(x)$ by $H^{Boot}_{n}(x)$ will become better for larger sample sizes $n.$ 
...
## Pinting the first six (head) elements of Xbar_boot:
head(Xbar_boot)
```
...
\bar{X}^*_{n,1},\dots,\bar{X}^*_{n,m}
$$
allow us to approximate (arbirary well as $m\to\infty$) the bootstrap distribution
$$
...
$$
arbitary well as $m\to\infty.$ Thus, by choosing a large $m$ (e.g. $m=10000$) we can proceed as if 
$$
...
If now
$$
H^{Boot}_{n}(x)\approx \Phi_{\sigma_0}(x)\quad\text{for all}\quad x,
$$
then we can directly use $H^{Boot}_{n}(x)$ (or $H^{Boot}_{n,m}(x)$) to do inference about $\mu_0,$ 
because this is then effectively the same as doing inference about $\mu_0,$ using the asymptotic normality result. 


The following `R` code demonstrates that:
$$
H^{Boot}_{n}(x)\approx \Phi_{\sigma_0}(x)\quad\text{for all}\quad x,
$$ 
