...
The wild bootstrap is a method for generating bootstrap samples that do not consist of resampling the original data or residuals as in @sec-bootPairs and @sec-bootResid. Rather, the wild bootstrap combines the data with random variables drawn from a known distribution to form a bootstrap sample. 

The wild bootstrap provides a way to deal with issues
such as heteroskedasticity of unknown form in fixed-design regression models or random-design models in which one conditions on the predictors. 
...
As in the residual bootstrap method in @sec-bootResid, the wild bootstrap uses the $X_i$'s from the original data. I.e., the $X_i$'s are not resampled. The wild bootstrap generates bootstrap samples 
$$
...
1. Let $\varepsilon_i=X_i^\top\hat\beta_n,$  $i=1,\dots,n$ be the OLS residuals. For each $i=1,\dots,n,$ let $F_i$ be the $i$-specific distribution of a discrete two-point random variable 
$$
...
1. Use either bootstrapping pairs (@sec-bootPairs) or bootstrapping residuals (@sec-bootResid) to generate $m$ (e.g. $m=10,000$) bootstrap realizations
$$
...
This basic bootstrap confidence interval provides an asymptotically (first order) accurate confidence interval for **homoskedastic errors,** but also for **heteroskedastic** errors. We even do not need to know whether the errors are homoskedastic or heteroskedastic. 
...
Let $\gamma_{jj,n}$ denote the $j$-th diagonal element of the matrix $(\frac{1}{n}\sum_{i=1}^n X_iX_i^T)^{-1}$, i.e.,
$$
\gamma_{jj,n}:=\left[\left(\frac{1}{n}\sum_{i=1}^n X_iX_i^\top\right)^{-1}\right]_{jj}.
$$
Then
$$
T_n=\sqrt{n}\frac{(\hat\beta_{n,j}-\beta_{0,j})}{\hat\sigma_n\sqrt{\gamma_{jj,n}}}
$$ 
with 
$$
\hat{\sigma}_n=\sqrt{\frac{1}{n}\sum_{i=1}^n\hat{\varepsilon}_i^2}
$$
is an asymptotically pivotal statistic; i.e. 
$$
T_n=\sqrt{n}\frac{(\hat\beta_{n,j}-\beta_{0,j})}{\hat\sigma_n\sqrt{\gamma_{jj,n}}}\rightarrow_d\mathcal{N}(0,1),\quad n\to\infty.
$$
...
with
$$
T^*_{n}=\sqrt{n}\frac{\hat\beta_{n,j}^*-\hat\beta_{0,j}}{\hat\sigma_n^* \sqrt{\gamma_{jj,n}}}
$$
where 
$$
...
\left\{
\begin{array}{ll}
\left[\left(\frac{1}{n}\sum_{i=1}^n X^\ast_i X^{\ast\top}_i\right)^{-1}\right]_{jj}&\textrm{when bootstrapping pairs}\\
\left[\left(\frac{1}{n}\sum_{i=1}^n X_iX_i^T\right)^{-1}\right]_{jj}&\textrm{when bootstrapping residuals}\\
\end{array}
\right.
$$
...
$$
\left[
  \hat\beta_{j,n}-\hat q^\ast_{1-\frac{\alpha}{2},n,j}\;\left(\frac{\hat\sigma_n \sqrt{\gamma_{jj,n}}}{\sqrt{n}}\right),\; 
  \hat\beta_{j,n}-\hat q^\ast_{\frac{\alpha}{2},n,j}\;\left(\frac{\hat\sigma_n \sqrt{\gamma_{jj,n}}}{\sqrt{n}}\right)
\right],
$$
where $\hat\beta_{n,j}$ and $\hat{\sigma}_n\,\sqrt{\gamma_{jj,n}}$ are computed from the original sample $\mathcal{S}_n$ and where the empirical quantiles $\hat q^*_{n,1-\frac{\alpha}{2}}$ and $\hat q^*_{n,\frac{\alpha}{2}}$ are computed from the bootstrap estimates $T^*_{n,1},T_{n,2}^*,\dots, T_{n,m}^*.$
...
Bootstrapping pairs allows, in principle, for heteroskedastic error terms. However, the above bootstrap-$t$ confidence interval is only usable under **homoskedastic errors**, even when bootstrapping pairs. 

Reason: The confidence interval uses the estimator of the standard error of $\hat{\beta}_{j,n}$ for the case of **homoskedastic errors:** 
$$
\widehat{\operatorname{SE}}(\hat{\beta}_{j,n}^\ast)=\frac{\hat\sigma_n^\ast \sqrt{\gamma_{jj,n}}}{\sqrt{n}}.
$$

In case of **heteroskedastic errors** one needs to use an appropriate version of $\widehat{\operatorname{SE}}(\hat{\beta}_{j,n}^\ast).$
::: 

### Statistical Hypothesis Testing 

In the following, we consider the **residual bootstrap** as introduced in @sec-bootResid. 
...
$$
T_n = \frac{\hat\beta_{j,n} }{\widehat{\operatorname{SE}}(\hat\beta_{j,n})},
$$
where 
...
and where in the case of homoskedastic error terms
$$
\widehat{\operatorname{SE}}(\hat\beta_{j,n})=\frac{\hat{\sigma}_n\sqrt{\gamma_{jj,n}}}{\sqrt{n}}.
$$
...
$$
\hat{\beta}^{H_0}_{n}=\left(\tilde{X}^\top\tilde{X}\right)^{-1}\tilde{X}^\top Y
$$
...
1. Draw independently and with replacement $n$ values from 
$$
