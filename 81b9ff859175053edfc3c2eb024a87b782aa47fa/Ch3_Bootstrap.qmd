...
### Bootstrapping Pairs: Bootstrap under Random Design {#sec-bootPairs}
...
### Residual Bootstrap: Bootstrap under Fixed Design {#sec-bootResid}
...
::: {.callout-tip}
There are furhter bootstrap procedures. For instance, one can adjust the residual bootstrap to allow for heteroskedastic errors which leads to the **Wild Bootstrap** or the **Multiplier Bootstrap** (see, e.g., Section 6 in @Horowitz_2001). These two methods are similar to the residual bootstrap since they generate new outcome variables $Y^\ast_i$ by generating new residuals, conditionally on the predictors $X_i$. However these boostrap methods generate new residuals by multiplying a real random variable $W_i\in\mathbb{R}$ to the observed (not resampled) residual $\varepsilon_i:$
$$
Y_i^\ast = X_i^\top \hat\beta_n + \underbrace{\hat\varepsilon_i \cdot W_i}_{=\hat\varepsilon_i^\ast},
$$ 
where $W_i$ has a specific distribution (Rademacher distribution, Mammen's distribution, Normal distribution, etc). 
<!-- 
This approach is very successful and also works in high-dimensional ($p$ as large as $n$ or larger) problems.  
-->
:::
