...
$$
However, computing $\operatorname{SE}(\hat\alpha)$ is here difficult due to the definition of $\hat\alpha$ in @eq-alphahat which contains variance estimates also in the denominator. 

::: {.callout-note}
# Conclusion: Why Bootstrap? 
In cases as described above, we are happy to use the **Basic Bootstrap Method** (@sec-BasicBootstrap) which allows us to approximate $\operatorname{SE}(\hat\alpha)$ simply by resampling from the data observed; i.e. without the need of an explicite formula of a consistent estimator of $\operatorname{SE}(\hat\alpha).$ The Basic Bootstrap Method is found to be as accurate as the standard asymptotic Normality results which, however, require an explicite formula of an estimator of $\operatorname{SE}(\hat\alpha)$ to become useful.


If we have a consistent estimator for the $\operatorname{SE}(\hat\alpha),$ then we can make use of this estimator by applying the **Bootstrap-$t$ Method** (@sec-BootT). The Bootstrap-$t$ Method is found to be **more accurate** than the standard asymptotic Normality results. 
:::
...
The Clivenko-Cantelli @thm-Clivenko-Cantelli states that $F_n$ is even **uniformly** over $\mathbb{R}$ a consistent estimator of $F.$
...
* $Var(W^*_{1,n}|\mathcal{S}_n)=\frac{1}{n}\hat{\sigma}_0^2=\frac{1}{n}\left(\frac{1}{n}\sum_{i=1}^n\left(X_i-\bar{X}_n\right)^2\right)$
...
$${#eq-Boot_tCI}
where $\hat\theta_n$ and $\hat v_n$ are the estimates of $\theta_0$ and $v_0$ based on the original sample $\mathcal{S}_n=\left\{X_1,\dots,X_n\right\}.$
...
$$ 
is more direct and hence more accurate ($v^*_n$ depends also on the bootstrap sample --- not on the original sample) than by the bootstrap law of 
$$
...
* Two-sided $(1-\alpha)\cdot 100\%$ confidence intervals $[L_n,U_n]$ are said to be **first-order accurate** if there exist some constants $c_1,c_2<\infty$ such that for sufficiently large $n$
$$
\begin{align*}
\left|P(\theta_0\not\in [L_n,U_n])-\alpha\right|\le \frac{c_1}{\sqrt{n}}
\end{align*}
$$
* Two-sided $(1-\alpha)\cdot 100\%$ confidence intervals $[L_n,U_n]$ are said to be **second-order accurate** if there exist some constants $c_2<\infty$ such that for sufficiently large $n$
$$
\begin{align*}
\left|P(\theta_0\not\in[L_n,U_n])-\alpha\right|\le \frac{c_2}{n}
\end{align*}
$$
...
* **homoskedastic** errors: $\mathbb{E}(\varepsilon_i^2|X_i)=\sigma^2$, $i=1,\dots,n$, for a constant $\sigma^2<\infty$ or 
* **heteroskedastic** errors: $\mathbb{E}(\varepsilon_i^2|X_i)=\sigma^2(X_i)<\infty$, $i=1,\dots,n.$

**Fixed Design:** 
$$
X_1, X_2, \dots, X_n
$$
are deterministic vectors in $\mathbb{R}^p$ and $\varepsilon_1,\dots,\varepsilon_n$ are i.i.d. random variables with zero mean $\mathbb{E}(\varepsilon_i)=0$ and **homoskedastic errors** $\mathbb{E}(\varepsilon_i^2)=\sigma^2$ for all $i=1,\dots,n.$
:::
:::
...
::: {.callout-tip}
For homoskedastic errors we have 
$$
\begin{align*}
Q
...
Bootstrapping regression estimates $\hat\beta_n$ is straightforward under a **random design** (@def-RandomFixedDesign). 
...
* Compute the approximate $(1-\alpha)\times 100\%$ confidence interval as in @eq-NPBootCI:
$$
...
X_i:=(\underbrace{X_{i1}}_{=1},X_{i2},\ldots,X_{ip})^T\in\mathbb{R}^p,
$$
under **fixed design** (@def-RandomFixedDesign), i.e., where $\varepsilon_1,\dots,\varepsilon_n$ are i.i.d. with zero mean $\mathbb{E}(\varepsilon_i)=0$ and **homoskedastic** errors
$$
...
The idea of the residual bootstrap is very simple: By assumption, the the error terms 
$$
...
$$
\hat\sigma^2_n\rightarrow_p \sigma^2
$$
as $n\to\infty.$
...
$$
and for large $n$ the distribution of 
$$
\sqrt{n}(\hat\beta_n - \beta_0)
$$ 
is approximately normal with mean zero and covariance matrix 
$$
\sigma^2 \Sigma_{X^\top X}^{-1}
$$
where $\Sigma_{X^\top X}=\mathbb{E}\left(X_iX_i^T\right);$ i.e.
$$
\sqrt{n}(\hat\beta-\beta)\to_d\mathcal{N}\left(0,\sigma^2 \Sigma_{X^\top X}^{-1}\right).
$$
...
$$
where $\hat\sigma^2_n\to\sigma^2$ as $n\to\infty.$ 

An appropriate central limit theorem argument implies that 
$$
\left.\sqrt{n}(\hat\beta^*_n-\hat\beta_n)\right|\mathcal{S}_n\to_d\mathcal{N}\left(0,\sigma^2 \Sigma_{X^\top X}^{-1}\right),
$$
as $n\to\infty.$
...
\approx\underbrace{\text{distribution}(\sqrt{n}(\hat\beta_n-\beta_0))}_{\mathcal{N}\left(0,\sigma^2 \Sigma_{X^\top X}^{-1}\right)}
$$
...
Compute the empirical $\frac{\alpha}{2}$ and $1-\frac{\alpha}{2}$ quantiles $\hat q_{\frac{\alpha}{2},j}$ and $\hat q_{1-\frac{\alpha}{2},j}$ of the  $m$ bootstrap estimates 
$$
\hat\beta_{n,1,j}^*,\hat\beta_{n,2,j}^*, \dots, \hat\beta_{n,m,j}^*.
$$

Compute the approximate $(1-\alpha)\times 100\%$ basic (nonparametric) bootstrap confidence interval as in @eq-NPBootCI:
$$
\left[2\hat\beta_{n,j}-\hat q_{1-\frac{\alpha}{2},j}, 
      2\hat\beta_{n,j}-\hat q_{ \frac{\alpha}{2},j }\right],
$$
...
Approximate the $\frac{\alpha}{2}$ and $1-\frac{\alpha}{2}$ quantiles $\hat q_{\frac{\alpha}{2},n,j}$ and $\hat q_{1-\frac{\alpha}{2},n,j}$  of the bootstrap distribution of
$$
T^*_n=\frac{\hat\beta_{n,j}^*-\hat\beta_{0,j}}{\hat\sigma_n^* \sqrt{\gamma_{jj}}}
$$
with 
$$
...
\right],
$$
where $\hat\beta_{n,j}$ and 
$\hat{\sigma}_n=\sqrt{\frac{1}{n}\sum_{i=1}^n\hat{\varepsilon}_i^2}$ are computed from the original sample $\mathcal{S}_n.$

::: {.callout-tip}
There are furhter bootstrap procedures. In case of heteroskedastic errors, for instance, there's also the "Wild Bootstrap" (also called Multiplier Bootstrap.) It is similar to the residual bootstrap since it generates new outcome variables $Y^\ast_i$ by generating new residuals. However it does so by multiplying a real random variable $W_i\in\mathbb{R}$ to the observed residuals
$$
Y_i^\ast = X_i^\top \hat\beta_n + \hat\varepsilon_n \cdot W_i,
$$ 
$W_i$ has a specific distribution (Rademacher distribution or Mammen's distribution). This approach is very successful and also works in high-dimensional ($p$ as large as $n$ or larger) problems. 
:::
...
$$
\left[\bar{X}_n-\hat{q}_{1-\alpha/2}\hat\sigma,  \bar{X}_n-\hat{q}_{\alpha/2}\hat\sigma\right],
$$
where $\hat\sigma=(n-1)^{-1}\sum_{i=1}^n(X_i-\bar{X}_n)^2$, and where $\hat{q}_{\alpha/2}$ and $\hat{q}_{1-\alpha/2}$ denote the $(\alpha/2)$ and the $(1-\alpha/2)$-quantiles of the bootstrap distribution of: 
$$
\frac{\bar{X}_n^\ast-\bar{X}_n}{\hat\sigma^\ast}.
$$


In the following we approximate the bootstrap distribution of $(\bar{X}_n^\ast-\bar{X}_n)/\hat\sigma^\ast$, compute the quantiles $\hat{q}_{\alpha/2}$ and $\hat{q}_{1-\alpha/2}$, and plot all of this:
