...
\end{align*} 
Thus, we can proceed by computing the mean of a discrete random variable such that 
\begin{align*}
\mathbb{E}^*(\bar{Y}^*) 
...
$$
Since we do not condition on the observed sample $\mathcal{S}_n,$ we have that 
$$
Y^* \sim F,
$$
because the unconditional $Y^*$ has the **same distribution as** $Y\sim F.$ Thus, we can proceed by computing the mean of $Y^*\sim F$ such that
$$
\begin{align*}
\mathbb{E}(\bar{Y}^*) 
