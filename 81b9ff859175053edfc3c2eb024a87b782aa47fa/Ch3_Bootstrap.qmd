...
::: {.callout-note}
# Recall: Inference using Classic Asymptotic Theory 
...
Hence we have shown that the nonparametric (standard) bootstrap is consistent for doing inference about $\mu_0$ using $\bar{X}_n.$
...
## Recall: Confidence Intervals using Classic Asymptotic Theory 
...
#### Nonparametric (Standard) Bootstrap Confidence Interval {-}

**Setup:**

* **Data:** i.i.d. random sample ${\cal S}_n:=\{X_1,\dots,X_n\}$ with $X_i\sim F$ for all $i=1,\dots,n$, where the distribution $F$ depends on the unknown parameter $\theta_0\in\mathbb{R}.$ 
