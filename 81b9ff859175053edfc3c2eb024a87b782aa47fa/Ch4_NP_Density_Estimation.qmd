...
* **Integrated Squared Error (ISE)**
  $$
  \mathrm{ISE}(\hat{f}_h(x))=\int (\hat{f}_h(x)-f(x))^2\,dx
  $$ 
  Caution: The $\mathrm{ISE}(\hat{f}_h(x))$ is a random quantity.
* **Mean Integrated Squared Error (MISE):**
  $$
  \mathrm{MISE}(\hat{f}_h)=\int\mathrm{MSE}(\hat{f}_h(x))\,dx=\int \mathbb{E}(\hat{f}_h(x)-f(x))^2\,dx
  $$
* **Asymptotic Mean Integrated Squared Error (AMISE):** 
  $$
  \mathrm{AMISE}(\hat{f}_h) = \mathrm{MISE}(\hat{f}_h) + o_P(1)
  $$
  I.e. the asymptotic approximation of the $\mathrm{MISE}.$



The minimum requirement on $\hat{f}_h$ is **consistency**. That is, the estimated density function $\hat{f}_h$ should approach the true (unknown!) density $f$, under the hypothetical assumption that the sample size grows indefinitely ($n \to \infty$),
...
**Proof-Strategy for showing pointwise consistency:** Show that the pointwise Mean Squared Error (MSE) for a given $x,$
$$
\mathrm{MSE}(\hat{f}_h(x)),
$$
converges to zero as $n\to\infty$ under the above assumptions. I.e., that
$$
\begin{align*}
...
&=
\mathbb{V}\left(\hat{f}_{h_n}(x)  \right)
+
...
\frac{ \sqrt{n\,h_n}\left(\hat{f}_{nh}(x) - \mathbb{E} \left( \hat{f}_{nh}(x) \right)\right)}{
     \sqrt{ \mathbb{V} \left( \hat{f}_{nh}(x) \right) }}
...
$$
\mathbb{E} \left( \hat{f}_{h_n}(x) \right)\neq f(x);
$$ 
...
$$
\begin{equation*}
 \sup_{x} \left| \hat{f}_{nh}(x) - f(x) \right| \rightarrow_P 0.
\end{equation*}
$$
