...
This inheritance of the properties of $K$ to the properties of $\hat{f}_{nh}$ also holds for the smoothness properties: 
...
Using a too large bandwidth (e.g. here $h=$ `r round(bw.SJ(inc76$income) * 5.5,  2)`):
...
We call a bandwidth choice method globally optimal, if it minimizes a global loss-function, which quantifies the estimation errors. 

Commonly used global loss-functions: 

* **Integrated Squared Error (ISE)**
  $$
  \mathrm{ISE}(\hat{f}_{nh})=\int (\hat{f}_{nh}(x)-f(x))^2\,dx
  $$ 
  Caution: The $\mathrm{ISE}(\hat{f}_{nh})$ is a *random* (stochastic) quantity.
...
The minimum requirement on $\hat{f}_{nh}$ is **consistency**. That is, the estimated density function $\hat{f}_{nh}$ should approach the true (unknown!) density $f$, under the hypothetical assumption that the sample size $n$ grows $(n \to \infty).$
...
$$ 
for finite $n.$ That is, when using the asymptotic normality result in finite samples,  
$$
\begin{equation*}
\left(\hat{f}_{nh}(x) \right)
   \overset{a}{\sim}\mathcal{N}\left(\mathbb{E} \left( \hat{f}_{nh}(x) \right),\,\frac{1}{n\,h_n}Var\left(\hat{f}_{nh}(x) \right)\right),
\end{equation*}
$$
we do inference about $\mathbb{E} \left( \hat{f}_{nh}(x) \right)\neq f(x).$
:::
:::
...
**Unknown quantity:** The global roughness of $f$
...
# Comparision: Parametric MSE rate of $\bar{X}_n$ for $\mu_0$
...
If one can assume that the underlying density $f$ does not deviate significantly, for instance, from a normal density, then a reasonable approximation of the optimal bandwidth for the true density $f$ can be obtained via the optimal bandwidth for the normal density.
...
$${#eq-sigma2estim2}
or the minimum of these two  (@eq-sigma2estim1 and @eq-sigma2estim2) estimators. 
...
(d) The density shown in @fig-norMix is a multi-modal denisty with four peaks. It is to be expected that:
$$
