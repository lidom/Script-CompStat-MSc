...
* $f$ is the density function of $\mathcal{N}(4,1)$
* $h(x) = 1_{(x\geq 3)}$
...
& = \frac{1}{n}\left[\mathbb{E}\left(\left(\frac{f(Y_i + 4.5)}{f_{\mathcal{Exp}\text{(1)}}(Y_i )}\right)^2\right)- p^2\right]\qquad [\text{using unbiasedness}]\end{align}
$$
...
& < c_{\max}\; \int_{4.5}^\infty   f(x)\;  dy = c_{\max}\; p < p
\end{align}
$$
where 
$$
