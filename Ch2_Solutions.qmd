...
\end{align*}
$$
with $X\sim\mathcal{Exp}(\lambda = 1).$
...
\end{align}
$$
where $X\sim f$ with $f$ denoting the density function of the standard normal distribution. 
...
n                   <- 100 
B                   <- 10000
theta_hat_estim_vec <- vector(mode = "double", length = B)
...
The importance function $g$ is a good/resonable choice since 
...
Importance sampling integration estimator, 
...
* $f$ is the density function of the standard normal distribution 
* $h(x) = 1_{(x\geq 3)}$
...
hfg <- function(x){
    tmp    <- dnorm(x, mean = 0, sd = 1)/dnorm(x, mean = 4, sd = 1) 
    tmp[x < 3] <- 0
    result <- tmp
    return(result)
}
...
n                      <- 100 
B                      <- 10000
theta_hat_IS_estim_vec <- vector(mode = "double", length = B)
...
\begin{align}
\hat{p}
& = \frac{1}{B}\sum_{i=1}^B 1_{(X\geq 4.5)}, 
\end{align}
$$
where $X\sim f.$
...
& = \mathbb{E}\left(\frac{1}{B}\sum_{i=1}^B 1_{(X\geq 4.5)}\right)\\[2ex]
& = \mathbb{E}\left(1_{(X\geq 4.5)}\right)\qquad [\text{i.i.d.}]\\[2ex] 
...
& = \mathbb{V}\left(\frac{1}{B}\sum_{i=1}^B 1_{(X\geq 4.5)}\right)\\[2ex]
& = \frac{1}{B} \mathbb{V}\left(1_{(X\geq 4.5)}\right)\qquad [\text{i.i.d.}]\\[2ex] 
& = \frac{p(1-p)}{B} \qquad [1_{(X\geq 4.5)}\sim\mathcal{Bern}(p)]
\end{align}
$$
Thus 
$$
\mathbb{V}\left(\hat{p}\right) = \frac{1}{B}(p-p^2)
$${#eq-Ch2Ex4-2}
...
\begin{align}
\hat{p}_{IS}
& = \frac{1}{B}\sum_{i=1}^B \frac{f(X_i)}{g(X_i)}, 
\end{align}
$$
...
\begin{align}
\hat{p}_{IS}
& = \frac{1}{B}\sum_{i=1}^B \frac{f(Y_i + 4.5)}{g(Y_i + 4.5)}\\[2ex]
& = \frac{1}{B}\sum_{i=1}^B \frac{f(Y_i + 4.5)}{f_{\mathcal{Exp}\text{(1)}}(Y_i )}, 
\end{align}
$$
where $Y_1,\dots,Y_n\overset{\text{i.i.d.}}{\sim}f_{\mathcal{Exp}\text{(1)}}.$ 
...
& = \mathbb{E}\left(\frac{1}{B}\sum_{i=1}^B \frac{f(Y_i + 4.5)}{f_{\mathcal{Exp}\text{(1)}}(Y_i )}\right)\\[2ex]
...
& = \int_{4.5}^\infty f(z)  dz\qquad[\text{subst.~}z=x+4.5]\\[2ex] 
& = p, 
\end{align}
$$
...
& = \mathbb{V}\left(\frac{1}{B}\sum_{i=1}^B \frac{f(Y_i + 4.5)}{f_{\mathcal{Exp}\text{(1)}}(Y_i )}\right)\\[2ex]
& = \frac{1}{B}\mathbb{V}\left(\frac{f(Y_i + 4.5)}{f_{\mathcal{Exp}\text{(1)}}(Y_i )}\right)\qquad [\text{i.i.d.}]\\[2ex] 
& = \frac{1}{B}\left[\mathbb{E}\left(\left(\frac{f(Y_i + 4.5)}{f_{\mathcal{Exp}\text{(1)}}(Y_i )}\right)^2\right)- \left(\mathbb{E}\left(\frac{f(Y_i + 4.5)}{f_{\mathcal{Exp}\text{(1)}}(Y_i )}\right)\right)^2\right]\qquad [\text{variance formula}]\\[2ex] 
& = \frac{1}{B}\left[\mathbb{E}\left(\left(\frac{f(Y_i + 4.5)}{f_{\mathcal{Exp}\text{(1)}}(Y_i )}\right)^2\right)- p^2\right]\qquad [\text{using unbiasedness}]\end{align}
$$
...
& = \int_{4.5}^\infty \frac{f^2(x)}{f_{\mathcal{Exp}\text{(1)}}(x-4.5)}  dy\qquad [\text{subst.~}x=y+4.5]\\[2ex]
& = \int_{4.5}^\infty \frac{f^2(y)}{g(y)} dy\qquad [\text{by definition of }g]\\[2ex]
& = \int_{4.5}^\infty \left(\frac{f(y)}{g(y)}\right) f(y) dy\\[2ex]
& < c_{\max}\; \int_{4.5}^\infty   f(y)\;  dy = c_{\max}\; p
\end{align}
$$
where 
$$
...
\begin{align}
\mathbb{V}\left(\hat{p}_{IS}\right)
& < \frac{1}{B}\left[c_{\max}\; p - p^2\right] < \frac{1}{B}\left[p - p^2\right] = \mathbb{V}\left(\hat{p}\right)
\end{align}
$$
...
::: {.callout-tip}
## 
Note: Using importance sampling there's no need to sample directly from $f.$ This is a further advantage of importance sampling over classical Monte Carlo integration. 
::: 